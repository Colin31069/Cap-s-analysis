...
## 2026-05-04 21:10 +08:00

- Summary: Added manual sample exclusion with a 20% floor cap, saved exclusion reasons in metadata, and made plotting/statistics skip excluded `.xlsx` files while reporting exclusions.
- Files: `Python version/skin_analysis/models.py`, `Python version/skin_analysis/exclusions.py`, `Python version/skin_analysis/metadata.py`, `Python version/skin_analysis/plotting.py`, `Python version/skin_analysis/statistics.py`, `Python version/skin_analysis/gui.py`, `Python version/tests/test_exclusions.py`, `Python version/tests/test_metadata.py`, `Python version/tests/test_plotting.py`, `Python version/tests/test_statistics.py`, `README.md`, `ANALYSIS_METHOD.md`, `CHANGELOG.md`
- Verification: `cd "Python version" && python3 -m unittest discover` (fails in global Python because the existing NumPy 2.4.4 environment is ABI-incompatible with installed pandas/Matplotlib); `cd "Python version" && ./.venv-gui/bin/python -m unittest discover` (pass; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && python3 -m py_compile main.py skin_analysis/*.py` (pass); `cd "Python version" && ./.venv-gui/bin/python -m py_compile main.py skin_analysis/*.py` (pass); `git diff --check` (pass)

## 2026-05-04 23:22 +08:00

- Summary: Added robust Delta % outlier review that flags candidate samples with leave-one-out median/MAD modified z-scores without automatically excluding them.
- Files: `Python version/skin_analysis/config.py`, `Python version/skin_analysis/models.py`, `Python version/skin_analysis/statistics.py`, `Python version/skin_analysis/gui.py`, `Python version/tests/test_statistics.py`, `README.md`, `ANALYSIS_METHOD.md`, `CHANGELOG.md`
...
## 2026-05-26 00:40 +08:00

- Summary: Added Python Tkinter curve splitting for lanolin/PBS combined traces, stored split metadata, made PBS plotting/statistics use the right-side segment, added lanolin/full segment viewing modes, and generated an HTML change report.
- Files: `Python version/skin_analysis/models.py`, `Python version/skin_analysis/metadata.py`, `Python version/skin_analysis/analysis.py`, `Python version/skin_analysis/plotting.py`, `Python version/skin_analysis/statistics.py`, `Python version/skin_analysis/gui.py`, `Python version/tests/test_metadata.py`, `Python version/tests/test_analysis.py`, `Python version/tests/test_plotting.py`, `README.md`, `docs/change_reports/version_20260526_0040.html`, `CHANGELOG.md`
- Verification: `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest tests.test_metadata tests.test_analysis tests.test_plotting tests.test_statistics` (pass, 63 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest discover` (pass, 75 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m py_compile main.py skin_analysis/*.py` (pass); `git diff --check` (pass)

## 2026-05-26 10:29 +08:00

- Summary: Added fixed 200-point PBS baseline pre-roll for split PBS traces, shifted PBS apply-window detection by the actual pre-roll duration, warned when pre-roll is short, and highlighted the pre-roll window during split-point picking.
- Files: `Python version/skin_analysis/config.py`, `Python version/skin_analysis/analysis.py`, `Python version/skin_analysis/gui.py`, `Python version/tests/test_analysis.py`, `Python version/tests/test_statistics.py`, `README.md`, `docs/change_reports/version_20260526_1029.html`, `CHANGELOG.md`
- Verification: `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest tests.test_analysis tests.test_statistics` (pass, 36 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest discover` (pass, 77 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m py_compile main.py skin_analysis/*.py` (pass); `git diff --check` (pass)

## 2026-05-26 10:46 +08:00

- Summary: Added plot-only manual drop alignment with per-sample/per-segment metadata, draggable GUI adjustment handles, sample/legend indicators, and statistics safeguards that keep analysis drop times unchanged.
- Files: `Python version/skin_analysis/models.py`, `Python version/skin_analysis/metadata.py`, `Python version/skin_analysis/plotting.py`, `Python version/skin_analysis/gui.py`, `Python version/tests/test_metadata.py`, `Python version/tests/test_plotting.py`, `Python version/tests/test_statistics.py`, `README.md`, `docs/change_reports/version_20260526_1046.html`, `CHANGELOG.md`
- Verification: `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest tests.test_metadata tests.test_plotting tests.test_statistics` (pass, 48 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest discover` (pass, 81 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m py_compile main.py skin_analysis/*.py` (pass); `git diff --check` (pass)

## 2026-05-26 11:02 +08:00

- Summary: Kept Simple plot legends to sample IDs only, moved warning/manual-drop suffixes to Detailed legends, and changed legend warning suffixes to ASCII to avoid missing-glyph boxes in Matplotlib.
- Files: `Python version/skin_analysis/plotting.py`, `Python version/tests/test_plotting.py`, `README.md`, `docs/change_reports/version_20260526_1102.html`, `CHANGELOG.md`
- Verification: `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest tests.test_plotting` (pass, 25 tests); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m unittest discover` (pass, 83 tests; SciPy emitted an expected Shapiro warning for identical test data); `cd "Python version" && PYTHONPYCACHEPREFIX=/private/tmp/codex_pycache ./.venv-gui/bin/python -m py_compile main.py skin_analysis/*.py` (pass); `git diff --check` (pass)
